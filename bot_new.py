...
class UserProfile:
    ...
    premium_until: str = ""
    settings: Dict = None
    telethon_session: Optional[str] = None # NEW: Telethon session string
...
class ForwardRule:
    ...
    use_user_account: bool = False # NEW: Whether to use user's connected account for this rule
...
class DatabaseManager:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS users (
                    ...
                    premium_until TEXT,
                    settings TEXT,
                    telethon_session TEXT -- NEW COLUMN
                )
            ''')
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS rules (
                    ...
                    schedule TEXT,
                    filters TEXT,
                    use_user_account BOOLEAN DEFAULT FALSE -- NEW COLUMN
                )
            ''')
            ...
            # --- NEW: Add missing columns if upgrading from older schema ---
            cursor.execute("PRAGMA table_info(users)")
            user_columns = [col[1] for col in cursor.fetchall()]
            if 'telethon_session' not in user_columns:
                cursor.execute("ALTER TABLE users ADD COLUMN telethon_session TEXT")
            
            cursor.execute("PRAGMA table_info(rules)")
            rule_columns = [col[1] for col in cursor.fetchall()]
            if 'use_user_account' not in rule_columns:
                cursor.execute("ALTER TABLE rules ADD COLUMN use_user_account BOOLEAN DEFAULT FALSE")
            conn.commit()
            # --- END NEW ---
    ...
    def get_user(self, user_id: int) -> Optional[UserProfile]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if row:
                ...
                # Adjust index for new column
                telethon_session = row[14] if len(row) > 14 else None 
                return UserProfile(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                ...
                (user_id, username, first_name, last_name, phone, role,
                 ...
                 is_banned, ban_reason, premium_until, settings, telethon_session)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
    ...
    def get_all_users(self) -> List[UserProfile]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                settings = json.loads(row[13]) if row[13] else {}
                telethon_session = row[14] if len(row) > 14 else None # Adjust index
    ...
    def get_rules(self, user_id: int = None) -> List[ForwardRule]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                # Adjust index for new column
                use_user_account = bool(row[14]) if len(row) > 14 else False
                rules.append(ForwardRule(
                    ...
                    use_user_account=use_user_account # NEW
                ))
            return rules
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                ...
                (rule_id, user_id, source_chat, target_chats, mode,
                 ...
                 created_date, last_triggered, trigger_count, schedule, filters, use_user_account)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
...
class TelethonManager:
    ...
    @classmethod
    async def get_client(cls, user_id: int, session_string: Optional[str]) -> Optional[TelegramClient]:
        if user_id in cls._active_clients and cls._active_clients[user_id].is_connected():
            return cls._active_clients[user_id]
        ...
        try:
            ...
            if await client.is_user_authorized():
                ...
                logger.warning(f"User {user_id} session string is invalid or expired. Disconnecting.")
                await client.disconnect()
                return None
        except Exception as e:
            logger.error(f"Error loading Telethon session for {user_id}: {e}")
            return None

    @classmethod
    async def start_login(cls, user_id: int, phone_number: str) -> Tuple[TelegramClient, str]:
    ...
    @classmethod
    async def complete_2fa(cls, user_id: int, password: str) -> str:
        client = cls._active_clients.get(user_id)
        if not client:
            raise ValueError("No active client session found for this user. Please restart login.")
    ...
    @classmethod
    async def disconnect_client(cls, user_id: int):
        if user_id in cls._active_clients:
            client = cls._active_clients[user_id]
            if client.is_connected():
                await client.disconnect()
            del cls._active_clients[user_id]
            logger.info(f"Telethon client for user {user_id} disconnected.")

    @classmethod
    async def get_chat_entity(cls, client: TelegramClient, chat_id: int):
        try:
            return await client.get_entity(chat_id)
        except Exception as e:
            logger.error(f"Telethon get_chat_entity failed for {chat_id}: {e}")
            return None

    @classmethod
    async def is_user_owner_of_chat(cls, client: TelegramClient, user_id: int, chat_id: int) -> bool:
        """Checks if the connected user is the owner of a given chat/channel."""
        try:
            entity = await cls.get_chat_entity(client, chat_id)
            if not entity:
                return False

            if isinstance(entity, Channel):
                # Fetch participants with admin rights to check for creator/owner status
                participants = await client.get_participants(entity, filter=TelethonUser, limit=None)
                for participant in participants:
                    if participant.id == user_id:
                        # Check if the user is the creator of the channel
                        if participant.creator:
                            return True
                        # Also check if they have full admin rights (can change info, post messages, etc.)
                        # This might indicate ownership or a very high level of admin.
                        if participant.admin_rights and \
                           participant.admin_rights.change_info and \
                           participant.admin_rights.post_messages and \
                           participant.admin_rights.add_admins and \
                           participant.admin_rights.delete_messages:
                            return True
                return False
            elif isinstance(entity, Chat):
                # For basic groups, there's no 'creator' field like channels.
                # You might need to check if the user is the 'owner' of the group.
                # This is more complex and might involve checking chat permissions or fetching full chat info.
                # For simplicity, we'll assume 'bypass' is primarily for channels where ownership is clearer.
                # If you need to support group ownership, you'd extend this logic.
                return False
            elif isinstance(entity, TelethonUser):
                return False
            return False
        except Exception as e:
            logger.error(f"Telethon is_user_owner_of_chat failed for user {user_id} in chat {chat_id}: {e}")
            return False
...
def get_string(lang_code: str, key: str) -> str:
    ...
    strings = {
        "en": {
            ...
            "connect_account_prompt": "📱 Please send your phone number (e.g., +12345678900) to connect your Telegram account. Use /cancel to abort.",
            "invalid_phone_format": "❌ Invalid phone number format. Please include country code (e.g., +12345678900).",
            "send_auth_code_prompt": "🔐 Please send the verification code you received in Telegram.",
            "send_2fa_password_prompt": "🔑 Two-factor authentication required. Please send your 2FA password.",
            "account_connected_success": "✅ Your Telegram account has been successfully connected!",
            "account_already_connected": "✅ Your Telegram account is already connected!",
            "account_disconnected_success": "✅ Your Telegram account has been disconnected.",
            "no_account_connected": "❌ No Telegram account is currently connected.",
            "telethon_error": "❌ An error occurred with your Telegram account connection: {error}. Please try again later.",
            "telethon_code_error": "❌ Invalid or expired code. Please try connecting your account again with /connect_account.",
            "telethon_flood_wait": "⚠️ Too many attempts. Please wait {seconds} seconds before trying again.",
            "rule_edit_use_user_account": "Use User Account for Forwarding",
            "rule_use_user_account_toggle": "✅ Rule {rule_id_short} will now use your connected Telegram account for forwarding: {status}.",
            "rule_use_user_account_not_connected": "❌ You must connect your Telegram account first to use this feature. Use /connect_account.",
            "rule_use_user_account_owner_check_failed": "❌ Failed to verify if you are the owner of the source chat using your connected account. Please ensure you are the owner or try without this option."
        },
...
(
    ...
    BAN_USER_REASON,
    # --- NEW TELETHON CONVERSATION STATES ---
    GET_PHONE_NUMBER_STATE, GET_AUTH_CODE_STATE, GET_2FA_PASSWORD_STATE
    # --- END NEW TELETHON CONVERSATION STATES ---
) = range(22) # Adjusted range for new states
    ...
    def decorator(func):
        ...
        async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if len(call_history[user_id]) >= max_calls:
                remaining_time = int(time_window - (now - call_history[user_id][0]))
                if update.message:
                    await update.message.reply_text(
                        f"⚠️ Rate limit exceeded. Please wait {remaining_time} seconds before trying again.",
...
class ForwardingEngine:
    """Real-time forwarding engine for automatic message forwarding"""
    ...
    def load_active_rules(self):
        """Load all active forwarding rules from database"""
        try:
            all_rules = db.get_rules()
            self.active_rules = [rule for rule in all_rules if rule.is_active]
    ...
    def reload_rules(self):
        """Reload active rules from database"""
        self.load_active_rules()

    async def process_message(self, update, context):
        """Process incoming message for auto-forwarding"""
        try:
            ...
            # Check if we have any active rules for this chat
            matching_rules = [rule for rule in self.active_rules if rule.source_chat == chat_id]
            
            if not matching_rules:
                logger.debug(f"No active rules found for chat {chat_id}")
                return
            
            logger.info(f"Found {len(matching_rules)} active rules for chat {chat_id}")
            
            # Process each matching rule
            for rule in matching_rules:
                try:
                    if await self._should_forward(message, rule):
                        logger.info(f"Forwarding message from chat {chat_id} using rule {rule.rule_id}")
    ...
    async def _should_forward(self, message, rule) -> bool:
        ...
        try:
            # Check schedule
            if rule.schedule and rule.schedule.get("enabled", False):
                current_time = datetime.now()
                start_time = datetime.strptime(rule.schedule["start_time"], "%H:%M").time()
                end_time = datetime.strptime(rule.schedule["end_time"], "%H:%M").time()
                ...
                # Check days of week
                if current_time.weekday() not in rule.schedule.get("days", []):
                    return False
            ...
            logger.error(f"Error in _should_forward: {e}")
            return False

    async def _forward_message(self, message: Message, rule: ForwardRule, bot: Bot):
        """Forward message according to rule settings"""
        try:
            # Forward to all target chats
            for target_chat in rule.target_chats:
                try:
                    if rule.use_user_account:
                        # Use user's connected account for forwarding
                        user_profile = db.get_user(rule.user_id)
                        if user_profile and user_profile.telethon_session:
                            client = await TelethonManager.get_client(rule.user_id, user_profile.telethon_session)
                            if client:
                                await self._forward_with_telethon(client, message, target_chat, rule.mode)
                            else:
                                logger.warning(f"User {rule.user_id} has no valid Telethon session")
                                continue
                        else:
                            logger.warning(f"User {rule.user_id} has no connected account")
                            continue
                    else:
                        # Use bot for forwarding
                        await self._forward_with_bot(bot, message, target_chat, rule.mode)

                    # Log successful forward
                    db.log_action(
                        user_id=rule.user_id,
                        action="auto_forward_success",
    ...
    async def _forward_with_bot(self, bot: Bot, message: Message, target_chat: int, mode: str):
        """Forward message using bot"""
        try:
            if mode == "with_tags":
                # Forward with original sender info
                await message.forward(target_chat)
            elif mode == "without_tags":
                # Forward without sender info
                if message.text:
                    await bot.send_message(target_chat, message.text)
                elif message.photo:
                    await bot.send_photo(target_chat, message.photo[-1].file_id, caption=message.caption)
                elif message.video:
                    await bot.send_video(target_chat, message.video.file_id, caption=message.caption)
                elif message.document:
                    await bot.send_document(target_chat, message.document.file_id, caption=message.caption)
                else:
                    await message.forward(target_chat)
            elif mode == "bypass":
                # Bypass mode - check if user is owner
            ...
            logger.error(f"Error in _forward_with_bot: {e}")
            raise e

    async def _forward_with_telethon(self, client: TelegramClient, message: Message, target_chat: int, mode: str):
        """Forward message using Telethon client"""
        try:
            # Convert telegram.Message to telethon message
            # This is a simplified version - you might need more complex conversion
            if message.text:
                await client.send_message(target_chat, message.text)
            elif message.photo:
                # Download and send photo
                photo_file = await message.photo[-1].get_file()
                await client.send_file(target_chat, photo_file.download_as_bytearray(), caption=message.caption)
            elif message.video:
                video_file = await message.video.get_file()
                await client.send_file(target_chat, video_file.download_as_bytearray(), caption=message.caption)
            elif message.document:
                doc_file = await message.document.get_file()
                await client.send_file(target_chat, doc_file.download_as_bytearray(), caption=message.caption)
            else:
                # For other types, try to forward
                await client.forward_messages(target_chat, message.message_id, from_peer=message.chat.id)

        except Exception as e:
            logger.error(f"Error in _forward_with_telethon: {e}")
            raise e

    async def manual_forward_message(self, bot: Bot, message: Message, mode: str, target_chats: List[int], user_id: int, use_user_account: bool = False):
        """Manually forward a message to specified targets"""
        try:
            user_profile = None
            client = None
            ...
            if use_user_account:
                user_profile = db.get_user(user_id)
                if user_profile and user_profile.telethon_session:
                    client = await TelethonManager.get_client(user_id, user_profile.telethon_session)

            for target_chat in target_chats:
                try:
                    if use_user_account and client:
                        await self._forward_with_telethon(client, message, target_chat, mode)
                    else:
                        await self._forward_with_bot(bot, message, target_chat, mode)

                    # Log successful forward
                    db.log_action(
                        user_id=user_id,
                        action="manual_forward_success",
                        details=f"Mode: {mode}",
                        source_chat=message.chat.id,
                        target_chats=[target_chat],
                        status="success"
            ...
            logger.error(f"Error in manual_forward_message: {e}")
            raise e
...
async def create_rule_mode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # NEW: Ask about using user account for forwarding
    user_profile = db.get_user(user_id)
    if user_profile and user_profile.telethon_session:
        keyboard = [
            [InlineKeyboardButton("✅ Yes, use my account", callback_data="use_user_account_yes")],
            [InlineKeyboardButton("❌ No, use bot account", callback_data="use_user_account_no")],
            [InlineKeyboardButton(get_string(user_lang, "cancel_operation"), callback_data="cancel_conversation")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await safe_edit_message(
            ...
            "Do you want to use your connected Telegram account for this rule? (Recommended for 'bypass' mode)",
            parse_mode=ParseMode.HTML,
            reply_markup=reply_markup
        )
        return CREATE_RULE_SOURCE # Move to next state to capture this choice
    else:
        context.user_data['new_rule_use_user_account'] = False
        await safe_edit_message(
            query,
            f"Selected mode: <code>{escape_html(mode)}</code>\n\n"
            "Now, please send the <b>source chat ID</b> (e.g., <code>-1001234567890</code>).",
            parse_mode=ParseMode.HTML
        )
...
async def create_rule_source(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # NEW: Handle callback for use_user_account choice
    if update.callback_query:
        ...
        use_user_account_choice = query.data == "use_user_account_yes"
        context.user_data['new_rule_use_user_account'] = use_user_account_choice

        await safe_edit_message(
            query,
            f"Using user account: <code>{use_user_account_choice}</code>\n\n"
            "Now, please send the <b>source chat ID</b> (e.g., <code>-1001234567890</code>).",
            parse_mode=ParseMode.HTML
        )
        return CREATE_RULE_SOURCE # Stay in this state to get source chat ID
    
    # Existing logic for source chat ID input
    try:
...
async def create_rule_targets(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    use_user_account = context.user_data.get('new_rule_use_user_account', False) # NEW

    # NEW: If use_user_account is true, verify user is owner of source chat
    if use_user_account and mode == 'bypass':
        user_profile = db.get_user(user_id)
        telethon_client = await TelethonManager.get_client(user_id, user_profile.telethon_session)
        if not telethon_client:
            await update.message.reply_text(get_string(user_lang, "rule_use_user_account_not_connected"))
            context.user_data.clear()
            return ConversationHandler.END
        
        is_owner = await TelethonManager.is_user_owner_of_chat(telethon_client, user_id, source_chat)
        if not is_owner:
            await update.message.reply_text(get_string(user_lang, "rule_use_user_account_owner_check_failed"))
            context.user_data.clear()
            return ConversationHandler.END
...
👤 <b>Use User Account:</b> {'✅ Yes' if use_user_account else '❌ No'}
...
async def edit_rule_start(update: Update, context: ContextTypes.DEFAULT_TYPE, rule_id: str = None) -> int:
    ...
    # Determine message_to_reply based on update type
    message_to_reply = update.effective_message

    # Prioritize rule_id from callback_query.data if available
    if update.callback_query and update.callback_query.data.startswith("rule_settings_"):
        rule_id = update.callback_query.data.replace("rule_settings_", "")
    elif not rule_id and context.args:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 Mode", callback_data=f"edit_rule_mode_{rule_id}"),
         InlineKeyboardButton("👤 Use User Account", callback_data=f"edit_rule_use_user_account_{rule_id}")], # NEW
    ...
    use_user_account_status = "✅ Yes" if rule.use_user_account else "❌ No" # NEW
...
• <b>Use User Account:</b> {escape_html(use_user_account_status)}
...
async def edit_rule_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    data = query.data
    # Extract rule_id from callback data if it's not already in context.user_data
    rule_id = context.user_data.get('rule_id')
    if not rule_id and '_' in data: # Attempt to extract from data if not found
        parts = data.split('_')
        if len(parts) > 2 and parts[-2] == 'rule': # e.g., edit_rule_toggle_active_RULEID
            rule_id = parts[-1]
        elif len(parts) > 1 and parts[-1].startswith('rule'): # e.g., rule_settings_RULEID
            rule_id = parts[-1]
    ...
    elif data.startswith("edit_rule_use_user_account"): # NEW: Toggle use_user_account
        user_profile = db.get_user(user_id)
        if not user_profile or not user_profile.telethon_session:
            await query.answer(get_string(user_lang, "rule_use_user_account_not_connected"), show_alert=True)
            return EDIT_RULE_SELECT # Stay in current menu

        rule.use_user_account = not rule.use_user_account
        
        # If enabling bypass mode with user account, perform owner check
        if rule.use_user_account and rule.mode == 'bypass':
            telethon_client = await TelethonManager.get_client(user_id, user_profile.telethon_session)
            if not telethon_client:
                await query.answer(get_string(user_lang, "rule_use_user_account_not_connected"), show_alert=True)
                rule.use_user_account = False # Revert if client not available
            else:
                is_owner = await TelethonManager.is_user_owner_of_chat(telethon_client, user_id, rule.source_chat)
                if not is_owner:
                    await query.answer(get_string(user_lang, "rule_use_user_account_owner_check_failed"), show_alert=True)
                    rule.use_user_account = False # Revert if not owner
        
        db.save_rule(rule)
        forwarding_engine.reload_rules()
        status = "Yes" if rule.use_user_account else "No"
        text = get_string(user_lang, "rule_use_user_account_toggle").format(rule_id_short=escape_html(rule_id[:8]), status=escape_html(status))
        markup = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data=f"rule_settings_{rule_id}")]])
        await safe_edit_message(query, text, reply_markup=markup, parse_mode=ParseMode.HTML)
        return EDIT_RULE_SELECT
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    escaped_telethon_status = escape_html(telethon_status)
    ...
    welcome_text = (
        ...
        f"🔗 <b>Telegram Account:</b> {escaped_telethon_status}\n\n" # NEW
        "<b>🎯 Three Forwarding Modes:</b>\n"
        "• <code>with_tags</code> - Keep original sender info\n"
        "• <code>without_tags</code> - Clean forwarding\n"
        "• <code>bypass</code> - Bypass restrictions (requires connected account & ownership)\n\n" # NEW
        ...
        "• Batch operations\n"
        "• Custom templates\n"
        "• Real-time monitoring\n" # Added from comprehensive feature set
        "• Advanced reporting\n" # Added from comprehensive feature set
        "• Custom notifications\n" # Added from comprehensive feature set
        "• Integration capabilities\n" # Added from comprehensive feature set
        "• Extended logging\n" # Added from comprehensive feature set
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("🔗 Connect Telegram Account", callback_data="connect_account") # NEW
        ],
        [
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "/connect_account - Connect your Telegram account for advanced forwarding\n" # NEW
        "/disconnect_account - Disconnect your Telegram account\n" # NEW
        ...
        "• <code>bypass</code> - Bypass restrictions + clean (requires connected account & ownership)\n\n" # NEW
    ...
    help_text += (
        ...
        "• Use <code>/forward</code> by replying to a message.\n" # Added this specific tip
        "• For <code>/forward</code>, the format is <code>/forward &lt;mode&gt; &lt;target_chat1&gt; [target_chat2] ... [true/false for use_user_account]</code>\n\n" # Added this specific tip
        "<b>🆘 Support:</b>\n"
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("🔗 Account Setup", callback_data="connect_account") # NEW
        ],
        [
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_text = (
        ...
        f"• Telegram Account: {escape_html(telethon_status)}\n" # NEW
...
async def connect_account_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_profile and user_profile.telethon_session:
        if update.callback_query:
            await update.callback_query.answer("You already have a connected Telegram account. Use /disconnect_account to disconnect first.")
        else:
            await update.message.reply_text("✅ You already have a connected Telegram account. Use /disconnect_account to disconnect first.")
        return ConversationHandler.END
    
    # Send the initial message based on update type
    if update.callback_query:
        await update.callback_query.answer()
...
async def get_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Add "Please wait" message with emojis
    wait_msg = await update.message.reply_text("⏳ Please wait while connecting to server... 🔄")

    try:
        logger.info(f"Starting Telethon login for user {user_id} with phone {phone_number}")
        client, phone_code_hash = await TelethonManager.start_login(user_id, phone_number)
        context.user_data['telethon_phone_number'] = phone_number
        context.user_data['telethon_phone_code_hash'] = phone_code_hash
        
        # Edit the wait message to show success
        await wait_msg.edit_text("✅ Connected to server successfully! 🔐")
        
        await update.message.reply_text("🔐 Please send the verification code you received in Telegram.")
        logger.info(f"Successfully sent code request for user {user_id}")
        return GET_AUTH_CODE_STATE
    except FloodWaitError as e:
        logger.warning(f"FloodWaitError for user {user_id}: {e}")
        await wait_msg.edit_text(f"⚠️ Too many attempts. Please wait {e.seconds} seconds before trying again.")
        return ConversationHandler.END
    except Exception as e:
        logger.error(f"Telethon login start error for {user_id}: {e}")
...
async def get_auth_code(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text("❌ Invalid or expired code. Please try connecting your account again with /connect_account.")
        context.user_data.clear()
        return ConversationHandler.END
    except Exception as e:
        logger.error(f"Telethon login complete error for {user_id}: {e}")
...
async def get_2fa_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.error(f"Telethon 2FA error for {user_id}: {e}")
        await update.message.reply_text(f"❌ An error occurred with your Telegram account connection: {escape_html(str(e))}. Please try again later.")
...
async def disconnect_account_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_profile and user_profile.telethon_session:
        user_profile.telethon_session = None
        db.save_user(user_profile)
        await TelethonManager.disconnect_client(user_id)
        await update.message.reply_text("✅ Your Telegram account has been disconnected.")
    else:
        await update.message.reply_text("❌ No Telegram account is currently connected.")
# --- END NEW TELETHON ACCOUNT CONNECTION COMMANDS ---
...
async def forward_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await update.message.reply_text(
            ...
            "<b>Example:</b>\n"
            "<code>/forward without_tags -1009876543210 true</code> (reply to message)\n\n"
            "To use /forward, <b>reply to the message</b> you want to forward.",
            parse_mode=ParseMode.HTML
        )
        return
    
    # Parse arguments
    use_user_account_flag = False
    args_for_parsing = list(context.args)

    if args_for_parsing and args_for_parsing[-1].lower() in ['true', 'false']:
        use_user_account_flag = (args_for_parsing.pop().lower() == 'true')
    
    if len(args_for_parsing) < 2:
        await update.message.reply_text(
            "📝 <b>Forward Command Usage:</b>\n\n"
            "<code>/forward &lt;mode&gt; &lt;target_chat1&gt; [target_chat2] ... [true/false for use_user_account]</code>\n\n"
            "<b>Modes:</b>\n"
    ...
    mode = args_for_parsing[0]
    if mode not in ['with_tags', 'without_tags', 'bypass']:
        await update.message.reply_text("❌ Invalid mode. Use: <code>with_tags</code>, <code>without_tags</code>, or <code>bypass</code>", parse_mode=ParseMode.HTML)
        return
    
    try:
        target_chats = [validate_chat_id(chat) for chat in args_for_parsing[1:]]
    except ValueError as e:
        await update.message.reply_text(f"❌ Invalid chat ID format: {escape_html(str(e))}", parse_mode=ParseMode.HTML)
        return
    ...
    try:
        stats = await forwarding_engine.manual_forward_message(
            ...
            target_chats=target_chats,
            user_id=user_id,
            use_user_account=use_user_account_flag
        )
        
        result_text = "✅ <b>Forward Complete!</b>\n\n"
        result_text += f"📊 <b>Results:</b>\n"
        result_text += f"• Successful: {stats['successful']}\n"
        result_text += f"• Failed: {stats['failed']}\n"
        result_text += f"• Mode: {escape_html(mode)}\n"
        result_text += f"• Use User Account: {'✅ Yes' if use_user_account_flag else '❌ No'}\n\n"
        result_text += f"📤 <b>Source Chat ID:</b> <code>{escape_html(str(message_to_forward.chat.id))}</code>\n"
        result_text += f"🎯 <b>Target Chat IDs:</b> {', '.join(f'<code>{escape_html(str(t))}</code>' for t in target_chats)}"
        
        if stats['errors']:
            result_text += f"\n\n❌ <b>Errors:</b>\n" + '\n'.join(f"• {escape_html(error)}" for error in stats['errors'][:5])
...
async def rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, rule in enumerate(user_rules[:10], 1):
        ...
        rules_text += (
            ...
            f"• Use User Account: {escape_html(use_user_account_status)}\n" # NEW
...
async def manage_all_rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
    ...
    for rule in paginated_rules:
        ...
        user_account_emoji = "👤" if rule.use_user_account else "🤖" # NEW
        rule_buttons.append([InlineKeyboardButton(f"{status_emoji} {user_account_emoji} Rule {escape_html(rule.rule_id[:8])}... (Source: {escape_html(str(rule.source_chat))})", callback_data=f"rule_settings_{rule.rule_id}")]) # Changed to HTML
...
• Telegram Account: {escape_html(telethon_status)}
...
• Telethon API ID: {'✅ Configured' if API_ID else '❌ Missing'}
• Telethon API Hash: {'✅ Configured' if API_HASH else '❌ Missing'}
"""
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if update.message.from_user.is_bot or update.message.text and update.message.text.startswith('/'):
            return
        ...
        logger.info(f"Message received in chat {chat_id} from user {user_id}")
...
<b>Step 2: Connect Telegram Account (Optional, but recommended for 'bypass' mode)</b>
Use /connect_account to link your personal Telegram account.

<b>Step 3: Create Your First Rule</b>
Use /autoforward and follow the interactive steps.

<b>Step 4: Test Manual Forward</b>
Reply to any message and use <code>/forward without_tags -1009876543210</code>

<b>Step 5: Manage Rules</b>
...
• <code>bypass</code> - Bypass restrictions + clean copy (requires connected account & ownership)
...
• Scheduled forwarding
• Batch operations
• <b>Use your own Telegram account for forwarding (bypassing some restrictions)</b>
...
"""
    ...
    elif data == "connect_account": # NEW
        return await connect_account_command(update, context) # Start conversation
    elif data == "my_rules":
    ...
    # --- Owner Callbacks ---
        ...
        # OwnerConsole is not defined in this context, assuming it's a placeholder for backup logic
        # For now, directly call the backup logic
        try:
            ...
            # Backup JSON data (if any, though DB is primary now)
            # with open(DATA_FILE, 'w', encoding='utf-8') as f:
            #     json.dump({}, f) # Placeholder, assuming data is in DB
            
            # Backup SQLite database
            shutil.copyfile(DATABASE_URL, BACKUP_DIR / db_backup_name)
...
async def handle_setting_change_value(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if setting_key == "timezone":
        ...
        try:
            ...
            await update.message.reply_text(
                f"✅ Rate limit has been updated to <code>{escape_html(str(new_rate_limit))}</code> calls per minute.",  # Changed to HTML
                parse_mode=ParseMode.HTML,  # Changed to HTML
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Settings", callback_data="user_settings")]])
            )
        except ValueError:
            await update.message.reply_text("❌ Invalid input. Please enter a valid number for the rate limit.")
    else:
        await update.message.reply_text("❌ This setting cannot be changed via this menu.")
